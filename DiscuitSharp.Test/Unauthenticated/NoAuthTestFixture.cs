    ...
    public partial class NoAuthTestFixture : FakeHttpTestFixture
    {
        ...
        protected override (HttpStatusCode, string) Content(Uri? url, string Method, JsonObject? json)
             => (url?.PathAndQuery, Method) switch
             {
                 ...
                 { PathAndQuery: "/api/posts", Method: "GET" } => (HttpStatusCode.OK, $"{{\"posts\":{FakeHttpTestFixture.HomePosts},\"next\":\"17692e122def73f25bd757e0\"}}" ),
                 { PathAndQuery: "/api/posts?feed=home", Method: "GET" } => (HttpStatusCode.OK, $"{{\"posts\":{FakeHttpTestFixture.HomePosts},\"next\":\"17692e122def73f25bd757e0\"}}"),
                 { PathAndQuery: "/api/posts?feed=home&sort=hot", Method: "GET" } => (HttpStatusCode.OK, $"{{\"posts\":{FakeHttpTestFixture.HomePosts},\"next\":\"17692e122def73f25bd757e0\"}}"),
                 { PathAndQuery: "/api/posts?CommunityId=17692e122def73f25bd757e0", Method: "GET" } => (HttpStatusCode.OK, $"{{\"posts\":{PostsFeedByCommunity},\"next\":\"hh34jdf500dfsdf2qiimj\"}}"),
                 { PathAndQuery: "/api/posts?limit=1", Method: "GET" } => (HttpStatusCode.OK, $"{{\"posts\":[{ImagePost}],\"next\":\"17692e122def73f25bd757e0\"}}"),
